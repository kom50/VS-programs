...
public class CalculatorNew extends JFrame implements ActionListener {
    ...
    public CalculatorNew() {
        ...
        op = new char[] { '%', '/', '*', '+', '-', '.' };
        icon = new String[] { "pw", "C", "<-", "/", "7", "8", "9", "x", "4", "5", "6", "-", "1", "2", "3", "+", "%",
                "0", ".", "=" };
        jb = new JButton[20];
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (obj == jb[3]) {
            ...
            /*
             * if(!s.isEmpty()){ for(int i = 0; i < op.length; i++){ if(s.charAt(s.length()
             * - 1) == op[i]){ f = true; } } if(f == false) text.setText(s + "/"); }
             */
        }
        if (obj == jb[7]) {
            // boolean f = false;
        ...
        if (obj == jb[11]) {
            ...
            /*
             * if(!s.isEmpty()){ for(int i = 0; i < op.length; i++){ if(s.charAt(s.length()
             * - 1) == op[i]){ f = true; } } if(f == false) text.setText(s + "-"); } if(f ==
             * false && s.isEmpty()) //-5 + 7 text.setText(s + "-");
             */
        }
        if (obj == jb[15]) {
            boolean f = false;
    ...
    void eval(String exp) {
        String lop = new String(), rop = new String(), leftstr = new String(), rightstr = new String();
        // newstr = new String();
        int index = 0;
        String s = new String();
        int lst = -1;
        for (int j = 0; j < op.length; j++) {
            ...
            System.out.println(exp + "   " + index + "  " + reverse(lop) + "   " + rop + " result : " + s + " left : "
                    + leftstr + " right : " + rightstr);
            if (getIndex(leftstr, 0) != -1 && getIndex(rightstr, 0) != -1) {
                exp = leftstr + s + rightstr;
                System.out.println("1 : " + exp + "   " + index + "  " + reverse(lop) + "   " + rop + " result : " + s
                        + "  left : " + leftstr + " right : " + rightstr);
            } else if (getIndex(leftstr, 0) == -1 && getIndex(rightstr, 0) != -1) {
                exp = s + rightstr;
                System.out.println("2 : " + exp + "   " + index + "  " + reverse(lop) + "   " + rop + " result : " + s
                        + " left : " + leftstr + " right : " + rightstr);
            } else if (getIndex(leftstr, 0) != -1 && getIndex(rightstr, 0) == -1) {
                exp = leftstr + s;
                System.out.println("3 : " + exp + "   " + index + "  " + reverse(lop) + "   " + rop + " result : " + s
                        + " left : " + leftstr + " right : " + rightstr);
            }
